%% @author bigdat
%% @doc @todo Add description to lib_chan.
...
%% 			io:format("lianguangfu~p~n",[ArgC]),
...
authenticate(MM,Service,Secret,ArgC) ->
	...
	receive
		...
		{chan,MM,{challenge,C}} ->
			...
			receive
				{chan,MM,ack} -> ok;
				{chan,MM,authFail} -> wait_close(MM),{error,authFail};
				Other -> 
					{error,Other}
			end;
		{chan,MM,badService} ->
